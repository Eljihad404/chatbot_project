...
export default function DashboardOverview({ api, token, days = 30, refreshKey = 0, onError }) {
  const [loading, setLoading] = useState(true);
  const [metrics, setMetrics] = useState({ users: 0, activeUsers: 0, docs: 0, chats: 0, tokens: 0 });
  ...
  async function load() {
    try {
      ...
      if (!mRes.ok || !tsRes.ok) throw new Error("Failed to fetch dashboard overview data");
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={series}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="tokens" stroke="#4F46E5" strokeWidth={3} dot={{ r: 3 }} name="Tokens" />
            </LineChart>
          </ResponsiveContainer>
        </div>
        {!loading && series.length === 0 && (
          <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">No token usage data.</p>
        )}
      </section>
    </>
  );
