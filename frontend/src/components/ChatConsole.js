...
export default function ChatConsole({ api, token: tokenProp }) {
  ...
  function normalizeRole(role) {
    ...
    if (["assistant", "ai", "bot", "model", "system", "tool"].includes(s)) return "assistant";
    // fallback (be generous to user so prompts go right)
    return "user";
  }
  function formatTime(ts) { try { return new Date(ts).toLocaleString(); } catch { return ""; } }
  function copy(text) { navigator.clipboard.writeText(text).catch(() => {}); }
  function avatar(isUser) {
    const bg = isUser ? "bg-indigo-600" : "bg-gray-300 dark:bg-gray-600";
    const fg = isUser ? "text-white" : "text-gray-900 dark:text-white";
  ...
  async function loadMessages(chatId, { scroll = true } = {}) {
    ...
    try {
      ...
      // normalize role names defensively
      const normed = (data || []).map(m => ({ ...m, role: normalizeRole(m.role) }));
      setMessages(normed);
  ...
  // Always send as USER (so your bubble is on the right)
 async function sendReply() {
  ...
  // Add assistant placeholder (left)
  setMessages(prev => [...prev, {
  ...
  try {
    const res = await fetch(`${API}/admin/chats/${selectedChat}/stream`, {
      method: "POST",
    ...
    for (;;) {
      ...
      setMessages(prev => {
        ...
        copy[copy.length - 1] = { ...last, role: "assistant", text: assistantText };
        return copy;
      });
    }

    // refresh from server so ids/timestamps match DB and history aligns
    await loadMessages(selectedChat, { scroll: true });
    requestAnimationFrame(() => inputRef.current?.focus());
  } catch (err) {
    console.error(err);
    setNetError(err?.name === "AbortError" ? "Stream aborted" : (err?.message || "Network error"));
    // Fallback: reload messages so UI is consistent
    await loadMessages(selectedChat, { scroll: false });
  } finally {
  ...
  // Optional: edit/delete (assistant only)
  ...
  useEffect(() => { if (selectedChat) { loadMessages(selectedChat); requestAnimationFrame(() => inputRef.current?.focus()); } /* eslint-disable-line */ }, [selectedChat]);
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          >
            ...
            {!messagesLoading && messages.map((m, i) => {
              ...
              return (
                <div key={m.id || i} className={`w-full flex ${isUser ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-3xl flex items-start gap-2 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                    {avatar(isUser)}
                    <div className={`group rounded-2xl px-4 py-2 shadow-sm border text-[15px] leading-relaxed whitespace-pre-wrap break-words
                      ...
                      <div className="text-[11px] opacity-70 mb-1 flex items-center justify-between gap-4">
                        <span className={`${isUser ? "text-white/80" : "text-gray-500"}`}>
                          {role} • {formatTime(m.created_at)}
                        </span>
                        {!isUser && (
          ...
          <div className="mt-3 border rounded-2xl p-2 bg-white dark:bg-gray-800">
            <div className="flex gap-2 items-end">
              <input
                ...
                placeholder={selectedChat ? "Type a message… (Enter to send)" : "Select a chat, then type your message…"}
                className="flex-1 px-4 py-2 rounded-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 min-h-[44px]"
