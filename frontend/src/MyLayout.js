...
useEffect(() => {
  ...
  // If no chat selected, create one using the first words as title
  if (!id) {
    id = await createChat(text.slice(0, 48));
    if (!id) { setStreaming(false); return; }
  } else {
    // If this chat is still "New chat", auto-rename it using the first message
    ...
    if (thisChat && (!thisChat.title || thisChat.title === 'New chat')) {
      ...
      // persist on backend
      renameChat(id, newTitle).catch(() => {/* ignore */});
    }
  }
